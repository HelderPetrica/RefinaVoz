...
@router.post("/process/texto", response_model=ProcessResponse)
...
async def process_audio(
    ...
    """Pipeline a partir de áudio WAV: Transcrição Gemini -> Texto Raw -> LLM Refina."""
    trace_id = str(uuid.uuid4())
    logger.info(f"[{trace_id}] Recebendo arquivo de audio: {audio_file.filename}")
    
    audio_bytes = await audio_file.read()
    
    # Passo isolado: Transcrição
    try:
        raw_text = await transcribe_audio_with_gemini(audio_bytes, audio_file.content_type)
        logger.info(f"[{trace_id}] Áudio transcrito | chars={len(raw_text)}")
    except Exception as e:
    ...
    # O resto segue o pipeline de texto
    ctx = await bus.emit("pre_process", text=raw_text, mode=mode, trace_id=trace_id)
    cleaned_text = ctx.get("text", raw_text)
    applied_terms = ctx.get("applied_terms", [])
    image_bytes, image_mime_type, visual_description = await read_visual_context_image(visual_context_image)
